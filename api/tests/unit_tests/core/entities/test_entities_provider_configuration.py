...
def _bind_db_engine_to_sqlite(sqlite_engine: Engine) -> Iterator[None]:
...
def test_generate_credential_names_from_real_rows_and_tenant_isolation(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    _provider_credential(sqlite_session, name="API KEY 9")
    _provider_credential(sqlite_session, name="legacy")
    _provider_credential(sqlite_session, name="API KEY 50", tenant_id="other-tenant")
    _model_credential(sqlite_session, name="API KEY 4")
    assert configuration._generate_provider_credential_name(sqlite_session) == "API KEY 10"
    assert configuration._generate_custom_model_credential_name("gpt-4o", ModelType.LLM, sqlite_session) == "API KEY 5"


def test_validate_provider_credentials_reuses_hidden_secret(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    configuration.provider.provider_credential_schema = _secret_provider_schema()
    credential = _provider_credential(sqlite_session, encrypted_config='{"openai_api_key":"enc-old"}')
    factory = Mock()
...
def test_preferred_provider_state_updates_and_is_tenant_scoped(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    ...
    sqlite_session.add_all([other, current])
    sqlite_session.commit()
    assert configuration.switch_preferred_provider_type(ProviderType.SYSTEM, session=sqlite_session)
    sqlite_session.refresh(current)
...
def test_provider_record_duplicate_and_setting_helpers_use_real_session(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    provider = _provider_record(sqlite_session)
    _provider_record(sqlite_session, tenant_id="other-tenant")
    credential = _provider_credential(sqlite_session, name="Main")
    _provider_credential(sqlite_session, name="Main", tenant_id="other-tenant")
    ...
    sqlite_session.commit()
    assert configuration._get_provider_record(sqlite_session).id == provider.id
    assert configuration._check_provider_credential_name_exists("Main", sqlite_session)
    assert not configuration._check_provider_credential_name_exists("Main", sqlite_session, exclude_id=credential.id)
    assert configuration._get_provider_model_setting(ModelType.LLM, "gpt-4o", sqlite_session).id == setting.id


def test_create_provider_credential_persists_provider_and_rejects_duplicate(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    with (
    ...
    credential = sqlite_session.scalar(select(ProviderCredential).where(ProviderCredential.credential_name == "Main"))
    provider = sqlite_session.scalar(select(Provider).where(Provider.tenant_id == "tenant-1"))
...
def test_update_provider_credential_propagates_to_load_balancing(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    credential = _provider_credential(sqlite_session, name="Old")
    _provider_record(sqlite_session, credential_id=credential.id)
    lb_config = _load_balancing_config(
        sqlite_session, credential_id=credential.id, source=CredentialSourceType.PROVIDER
    )
    with (
    ...
    assert sqlite_session.get(ProviderCredential, credential.id).credential_name == "New"
    persisted_lb = sqlite_session.get(LoadBalancingModelConfig, lb_config.id)
    assert persisted_lb.name == "New"
    assert json.loads(persisted_lb.encrypted_config) == {"api_key": "enc-new"}


def test_switch_and_delete_provider_credentials_updates_persisted_state(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    configuration.preferred_provider_type = ProviderType.CUSTOM
    first = _provider_credential(sqlite_session, name="First")
    second = _provider_credential(sqlite_session, name="Second")
    provider = _provider_record(sqlite_session, credential_id=first.id)
    preferred_provider = TenantPreferredModelProvider(
        tenant_id="tenant-1",
        provider_name="openai",
        preferred_provider_type=ProviderType.CUSTOM,
    )
    sqlite_session.add(preferred_provider)
    sqlite_session.commit()
    first_id = first.id
    second_id = second.id
    ...
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderCredential, second_id) is None
    assert sqlite_session.get(ProviderCredential, first_id) is not None
    assert sqlite_session.get(Provider, provider_id).credential_id is None
    assert (
        sqlite_session.get(TenantPreferredModelProvider, preferred_provider_id).preferred_provider_type
        == ProviderType.SYSTEM
    )


def test_specific_provider_credential_decrypts_and_obfuscates(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    configuration.provider.provider_credential_schema = _secret_provider_schema()
    credential = _provider_credential(sqlite_session, encrypted_config='{"openai_api_key":"enc"}')
    with (
        patch("core.entities.provider_configuration.encrypter.decrypt_token", return_value="raw"),
...
def test_specific_provider_credential_logs_decrypt_failure_without_failing_request(
...
def test_validate_custom_model_credentials_reuses_hidden_secret(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    configuration.provider.model_credential_schema = _secret_model_schema()
    credential = _model_credential(sqlite_session, encrypted_config='{"openai_api_key":"enc-old"}')
    factory = Mock()
...
def test_create_update_and_delete_custom_model_credential(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    with (
    ...
    credential = sqlite_session.scalar(select(ProviderModelCredential))
    model = sqlite_session.scalar(select(ProviderModel))
    assert credential is not None
    ...
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderModelCredential, credential_id).credential_name == "Renamed"

    with _mock_cache_boundaries():
        configuration.delete_custom_model_credential(ModelType.LLM, "gpt-4o", credential_id)
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderModelCredential, credential_id) is None
    assert sqlite_session.get(ProviderModel, model_id) is None


def test_add_and_switch_custom_model_credential(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    first = _model_credential(sqlite_session, name="First")
    second = _model_credential(sqlite_session, name="Second")
    with _mock_cache_boundaries():
...
def test_model_settings_and_load_balancing_persist(sqlite_session: Session) -> None:
    configuration = _build_provider_configuration()
    with patch.object(configuration, "_invalidate_provider_configuration_cache"):
        configuration.disable_model(ModelType.LLM, "gpt-4o")
    persisted_setting = sqlite_session.scalar(select(ProviderModelSetting))
    ...
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderModelSetting, setting_id).enabled is True

    first = _provider_credential(sqlite_session, name="First")
    second = _provider_credential(sqlite_session, name="Second")
    _load_balancing_config(sqlite_session, credential_id=first.id, source=CredentialSourceType.PROVIDER)
    _load_balancing_config(sqlite_session, credential_id=second.id, source=CredentialSourceType.PROVIDER)
    with patch.object(configuration, "_invalidate_provider_configuration_cache"):
        configuration.enable_model_load_balancing(ModelType.LLM, "gpt-4o")
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderModelSetting, setting_id).load_balancing_enabled is True
    with patch.object(configuration, "_invalidate_provider_configuration_cache"):
        configuration.disable_model_load_balancing(ModelType.LLM, "gpt-4o")
    sqlite_session.expire_all()
    assert sqlite_session.get(ProviderModelSetting, setting_id).load_balancing_enabled is False


def test_provider_create_rolls_back_on_insert_failure(sqlite_session: Session, sqlite_engine: Engine) -> None:
    configuration = _build_provider_configuration()
    with (
        patch.object(ProviderConfiguration, "validate_provider_credentials", return_value={"api_key": "enc"}),
        _mock_cache_boundaries(),
        ...
        configuration.create_provider_credential({"api_key": "raw"}, "Main")
    assert sqlite_session.scalar(select(ProviderCredential)) is None
    assert sqlite_session.scalar(select(Provider)) is None


def test_custom_model_create_rolls_back_on_insert_failure(sqlite_session: Session, sqlite_engine: Engine) -> None:
    configuration = _build_provider_configuration()
    with (
        patch.object(ProviderConfiguration, "validate_custom_model_credentials", return_value={"api_key": "enc"}),
        _mock_cache_boundaries(),
        ...
        configuration.create_custom_model_credential(ModelType.LLM, "gpt-4o", {"api_key": "raw"}, "Main")
    assert sqlite_session.scalar(select(ProviderModelCredential)) is None
    assert sqlite_session.scalar(select(ProviderModel)) is None
